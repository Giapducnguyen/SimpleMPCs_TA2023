...
%% MPC Data
Qx = eye(2);        % State penalty matrix
Rx = 1;             % Control penalty matrix
Np = 16;            % Prediction horizon
...
%% Extended-state dynamics to include disturbance information
% % z(k) = [x(k); d(k)]
% % z(k+1) = Az * z(k) + Bz * u(k)

Az = [A,                       Bd;
      zeros(nd,size(A,2)),     eye(nd)];
Bz = [B; zeros(nd,size(B,2))];

Qz = blkdiag(Qx,0);     % Extended state penalty matrix
P = blkdiag(P,0);       % Extended terminal state penalty matrix
...
mpcMethod = 2; % offline; 2 - online

% % Offline Computation
U_rollOut = mpcPolicy([X_nom(:,1);d],Az,Bz,Qz,P,Rx,Np,set_xbar,set_ubar,set_xfbar);
...
for k = 1:Nsim
    ...
    if mpcMethod == 1           % offline
        ...
        U_online = mpcPolicy([X_nom(:,k);d],Az,Bz,Qz,P,Rx,Np,set_xbar,set_ubar,set_xfbar);
        u_mpc = U_online(1,:);
    end
    Exe_time(k) = toc;          % execution time logging

    % Propagate prediction model forward
    X_nom(:,k+1) = A*X_nom(:,k) + B*u_mpc + Bd*d;
    ...
    X_act(:,k+1) = A*X_act(:,k) + B*u_act + w + Bd*d;
...
% % epsilon-approximation of Disturbance invariant set
...
% % Constraint can be expressed by H-representation as:
% % Nominal state constraint:
% % set_xbar.A * x_nominal <= set_xbar.b

% % Nominal control constraint:
% % set_ubar.A * u_nominal <= set_ubar.b

% % Now the extended system state is z = [x; d]. Therefore: 
% % We append the Polyhedron expression to account for disturbance
F_zbar.A = [set_xbar.A, zeros(size(set_xbar.A,1),1)];
F_zfbar.A = [set_xfbar.A, zeros(size(set_xfbar.A,1),1)];

Uad.A = [kron(eye(Np),set_ubar.A);
         blkdiag(kron(eye(Np-1),F_zbar.A),F_zfbar.A)*B_bar];
Uad.b = [kron(ones(Np,1),set_ubar.b);
         [kron(ones(Np-1,1),set_xbar.b);set_xfbar.b]];
Uad.B = [zeros(Np*size(set_ubar.b,1),n) ; 
         blkdiag(kron(eye(Np-1),F_zbar.A),F_zfbar.A)*A_bar];
